...
import { CompiledData } from "../../../model-dsfr/types";
import { createObjectThatThrowsIfAccessed, createUsecaseContextApi } from "redux-clean-architecture";
import { Mutex } from "async-mutex";
import { assert } from "tsafe/assert";
import type { SoftwareRow, ReferentRow } from "../../../model-dsfr/types";
import { buildCatalog } from "../../../model-dsfr/buildCatalog";
import { buildServices } from "../../../model-dsfr/buildServices";
...
type SoftwareRowEditableByForm = Pick<SoftwareRow, SoftwareRowKeyEditableByForm>;
    ...
    "initialize":
        ...
        async (...args) => {
            ...
            evtAction
                ...
                .toStateful()
                .pipe(() => [
                    Buffer.from(JSON.stringify(selectors.compiledDataWithoutReferents(getState()), null, 2), "utf8")
                ])
                .attach(buff => (getContext(extraArg).sillJsonBuffer = buff));
        },
    "updateStateRemoteAndLocal":
...
export const thunks = {
    ...
    "createReferent":
        ...
        async (...args) => {
            ...
            await mutex.runExclusive(async () => {
                ...
                const { referentRows, softwareRows, softwareReferentRows } = newDb;

                {
                    const softwareRow = softwareRows.find(({ id }) => id === softwareId);
                ...
                if (referentRows.find(({ email }) => referentRow.email === email) === undefined) {
                    referentRows.push(referentRow);
                }

                const softwareReferentRow = softwareReferentRows.find(
                    softwareReferentRow =>
                        referentRow.email === softwareReferentRow.referentEmail &&
    ...
    "userNoLongerReferent":
        (params: { email: string; softwareId: number }) =>
        async (...args) => {
            const [dispatch, getState, extraArg] = args;
            ...
            await mutex.runExclusive(async () => {
                ...
                const { referentRows, softwareReferentRows, softwareRows } = newDb;
                ...
                const index = softwareReferentRows.findIndex(
                    softwareReferentRow =>
                        softwareReferentRow.softwareId === softwareId && softwareReferentRow.referentEmail === email
                );
                ...
                if (
                    softwareReferentRows.find(softwareReferentRow => softwareReferentRow.referentEmail === email) ===
                    undefined
                ) {
                    const index = referentRows.findIndex(referentRow => referentRow.email === email);
            ...
            const { software } = await mutex.runExclusive(async () => {
                ...
                assert(
                    softwareRows.find(s => {
                        ...
                        return t(s.name) === t(softwareRowEditableByForm.name);
                    }) === undefined,
                ...
                const software = getState().webApi.compiledData.catalog.find(software => software.id === softwareId);
    ...
    "updateSoftware":
        (params: { softwareId: number; softwareRowEditableByForm: SoftwareRowEditableByForm; email: string }) =>
        async (...args) => {
            const [dispatch, getState, extraArg] = args;
            ...
            const { software } = await mutex.runExclusive(async () => {
                ...
                assert(
                    softwareReferentRows.find(({ referentEmail }) => referentEmail === email) !== undefined,
                    "The user is not a referent of this software"
                );

                const index = softwareRows.findIndex(softwareRow => softwareRow.id === softwareId);
            ...
            await mutex.runExclusive(async () => {
                ...
                if (isDeletion) {
                    ...
                    softwareReferentRows
                        .filter(softwareReferentRow => softwareReferentRow.softwareId === id)
                        .forEach(softwareReferentRow =>
                            softwareReferentRows.splice(softwareReferentRows.indexOf(softwareReferentRow), 1)
                        );
                ...
                softwareRows[index].dereferencing = structuredClone(dereferencing);
    ...
    "changeUserAgencyName":
        (params: { email: string; newAgencyName: string }) =>
        async (...args) => {
            const [dispatch, getState, extraArg] = args;
            ...
            await mutex.runExclusive(async () => {
                ...
                const referent = referentRows.find(row => row.email === email);
    ...
    "updateUserEmail":
        (params: { email: string; newEmail: string }) =>
        async (...args) => {
            const [dispatch, getState, extraArg] = args;
            ...
            await mutex.runExclusive(async () => {
                ...
                softwareReferentRows
                    ...
                    .forEach(softwareReferentRow => (softwareReferentRow.referentEmail = newEmail));
    ...
    "deleteService":
        (params: { serviceId: number; reason: string; email: string }) =>
        async (...args) => {
            const [dispatch, getState, extraArg] = args;
            ...
            await mutex.runExclusive(async () => {
                ...
                const index = serviceRows.findIndex(serviceRow => serviceRow.id === serviceId);
    ...
    "addService":
        (params: { serviceFormData: ServiceFormData; email: string }) =>
        async (...args) => {
            const [dispatch, getState, extraArg] = args;
            ...
            const { service } = await mutex.runExclusive(async () => {
                ...
                const serviceId =
                    newDb.serviceRows.map(({ id }) => id).reduce((prev, curr) => Math.max(prev, curr), 0) + 1;

                const { agencyName, serviceUrl, description, deployedSoftware, ...rest } = serviceFormData;
                ...
                serviceRows.push({
                    ...
                    ...(deployedSoftware.isInSill
                        ? (() => {
                              const { isInSill, softwareSillId, ...rest } = deployedSoftware;
                        ...
                        : (() => {
                              const { isInSill, softwareName, ...rest } = deployedSoftware;
                ...
                const service = getState().webApi.compiledData.services.find(service => service.id === serviceId);
    ...
    "updateService":
        (params: { serviceId: number; serviceFormData: ServiceFormData; email: string }) =>
        async (...args) => {
            const [dispatch, getState, extraArg] = args;
            ...
            const { service } = await mutex.runExclusive(async () => {
                ...
                const index = serviceRows.findIndex(service => service.id === serviceId);
...
export const selectors = (() => {
    ...
    const referentsBySoftwareId = createSelector(
        sliceState,
        (state): Record<number, CompiledData.Software.WithReferent["referents"]> => {
            const { compiledData } = state;

            return Object.fromEntries(compiledData.catalog.map(({ id, referents }) => [id, referents]));
        }
    );

    const compiledDataWithoutReferents = createSelector(sliceState, (state): CompiledData<"without referents"> => {
        const { compiledData } = state;
